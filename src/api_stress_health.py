...
with open("stress_xgb_tuned_model.pkl", "rb") as f:
    model = pickle.load(f)
...
def retrieve_resources(risk_increasing, top_k=2):
    query = f"Indicators related to {', '.join(risk_increasing)}"
...
- Anadn: Adn captures changes in metabolic + neurogenic activity, making it a strong stress indicator
...
Factors leading to stressed conditions:
{risk_increasing}
...
@app.route("/PredictStressByHealth", methods=["POST"])
def predict():
    try:
        ...
        prediction_label = "Normal" if pred_class == 0 else "Stressed"
